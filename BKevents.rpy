...
label before_main_menu(): # Will show before main menu (standard Ren'py label)
    ...
    if old_gp:
        if len(old_gp) == 1:
            ...
            $ text1 = "找不到以下的女孩包: {b}{color=[c_red]}" + and_text(old_gp) + "{/color}{/b}。它们将从组合中删除。"
    ...
    if new_gp:
        if len(new_gp) == 1:
            $ text1 = "一个新的女孩包: {b}{color=" + c_green + "}" + new_gp[0] + "{/color}{/b} 已发现！你想怎么做？"
        else:
            $ text1 = "发现了 " + str(len(new_gp)) + " 个新的女孩包:{b}{color=" + c_green + "}" + and_text(new_gp) + "{/color}{/b}。你想怎么做？"

        if renpy.call_screen("yes_no", text1, "更新所有女孩组合包", "不要更新（手动更新）"):
            python:
                for mix in persistent.girl_mix.values():
                    for gp in new_gp:
                        if gp not in mix:
...
label break_extractor(resource):
    ...
    call receive_item(item_dict["西梅利亚废料"], msg="You were able to scavenge a piece of %s from the wreck.", use_article=False) from _call_receive_item_16
