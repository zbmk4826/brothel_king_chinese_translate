        ...
        def update_mods(self): # Fun fact: renpy.call breaks python blocks, but not renpy.call_screen or renpy.say
            ...
            for mod in self.active_mods.values():
                ...
                if mod.name not in detected_mods.keys():
                    if renpy.call_screen("yes_no","找不到" + mod.full_name + "。是否要为此游戏停用此MOD（推荐）？"):
            ...
            for name, mod in detected_mods.items():
                if mod.active:
                    if name not in self.active_mods.keys():
                        if renpy.call_screen("yes_no", "新Mod已经被激活：" + mod.full_name + ". 是否为此游戏启用此MOD？"):
                            self.activate_mod(mod)
...
#                    renpy.say("System", event_color["bad"] % ("Failure to start " + mod.name) + " (calling " + mod.init_label + " label failed).")
    ...
    class Main(object): #Attributes: name, job, god, pictures, decisions, inventory, girl inventory, character
        ...
        def load_pics(self):
            ...
            if not self.pics:
                raise AssertionError, "没有找到主角的图片. 请检查game/MC文件夹。"
            else:
    ...
    class NPC(): # Attributes: name, pictures, inventory, girl inventory, character
        ...
        def upgrade_shop(self, cost, upgrade, forced=False): # Where cost and upgrades are tuples
            if forced or MC.has_resource(*cost):
                ...
                renpy.call_screen("OK_screen", "Missing resources", "你缺少此次升级所需的材料（%s %s）。" % (cost[1], cost[0]))
                return False
...
#             self.satisfied = False
        ...
        def get_reputation_comment(self, chg):
            ...
            if self.base_rating == 0:
                ...
                comment = event_color["special"] % rand_choice(["我度过了一生中最美好的时光。", "一切都是那么完美。", "有史以来最棒的夜晚！我已经筋疲力尽了。", "这地方太神奇了。五星好评！"])
                ...
                if self.service_dict["entertained"] >= 2:
                    ...
                    pos_comments += ["我获得一些娱乐。", "我在等待中得到了乐趣。", "一个女孩为我表演。"]
                    neg_comments.append("演艺人员可以做得更好一些。")
                else:
                    neg_comments += ["没有什么娱乐活动。", "我在等待时感到很无聊。", "没有什么乐子。真烦人……"]

                if self.service_dict["laid"] >= 2:
                    pos_comments.append("性爱真的很赞。")
                elif self.service_dict["laid"] == 1:
                    pos_comments += ["我有了新欢。", "有个妓女服侍了我。", "我有%s。" % self.got_sex_act]
                ...
                if self.service_dict["favorite sex act"] > 1:
                    pos_comments.append("我得到了我最喜欢的性爱服务。")
                else:
                    neg_comments.append("没有我最喜欢的性爱服务。")
...
#             self.maint_value = 1 # maint_value is antiquated
        ...
        def upgrade_master_bedroom(self, target_level="auto"):
            ...
            if MC.has_gold(master_bedrooms[target_level].cost):
                if renpy.call_screen("yes_no", "你确定为升级你的卧室而花费 " + str(master_bedrooms[target_level].cost) + "金币吗？"):
...
#                     test_achievement("upgrades")
                ...
                renpy.say(sill, "对不起，主人，您没有足够的金币。")
        ...
        def buy_furniture(self, furn):
            ...
            if self.current_building:
                ...
                if furn.built:
                    renpy.say(carpenter, "你已经有了一个了。我不认为你还需要第二个。")
        ...
        def get_description(self):
            ...
            if self.type in ("special", "personality"):
                ...
                if target == "naked":
                    ...
                    text1 = "可能会因为她的性欲、服从、体质或敏感而提高客人的满意度"
                    ...
                    text1 += "如果她只激活一个性行为，小费和声誉收益+25%(不包括双飞和群交)"
            ...
            if self.type == "allow":
                if target.endswith("preference"):
                    ...
                    target = "性虐偏爱" if target == "fetish preference" else target
                    ...
                    text1 += "允许'" + setting_name_dict[target] + "'光顾你的青楼"
...
#                 else:
            ...
            target = "性虐职业经验收益" if target == "fetish jp gains" else target
            target = "舞娘职业经验收益" if target == "dancer jp gains" else target
            target = "职业经验收益" if target == "jp gains" else target
            ...
            target = "性虐技能收益" if target == "fetish gains" else target
            ...
            target = "体质收益" if target == "constitution gains" else target
            ...
            target = "性虐偏爱增加" if target == "fetish preference increase" else target
            ...
            target = "性虐职业经验得分" if target == "fetish jp bonus" else target
            ...
            target = "性虐职业经验" if target == "fetish jp" else target
            ...
            target = "性虐结果评价" if target == "fetish results" else target
...
#        """This class is for inventories both with items and girls."""
    ...
    class Room(object):
        ...
        def get_description(self):
            if self.type == "master":
                if self.level > 1:
                    desc = "{font=[style.default.font]}最多可容纳" + str(self.level) + " 位女孩进行训练. 进入你的卧室的女孩每天晚上都接受免费训练。"
                elif self.level == 1:
                    desc = "{font=[style.default.font]}最多可容纳1位女孩接受培训. 进入你的卧室的女孩每天晚上都接受免费训练。"
                else:
...
#                 cost = 1
    ...
    class Personality(object): # Personality archetype used to semi-randomize girl personality attributes and specific dialogue
        ...
        def __init__(self, name,label, attributes, personality_dialogue_only=None, dialogue_personality_weight=3, dialogue_attribute_weight=1, custom_dialogue_label=None, description="", often_stories=None, rarely_stories=None, never_stories=None):
