                ...
                if district.rank >= 2 and renpy.random.random() <= 0.12: # 8% chance of getting a resource, 4% Cimerian
                    ...
                    if dice(6) >= 5:
                        if game.chapter >= 3 and dice(6) >= 6:
                            call receive_item(search_items("西梅利亚文物")[0], msg="你收到了一个罕见的 %s 。", use_article=False) from _call_receive_item
                        else:
                            call receive_item(search_items("西梅利亚废料")[0], msg="你收到一个 %s 。", use_article=False) from _call_receive_item_1
...
#    $ renpy.start_predict_screen("item_tab", MC, shop, "shop")
    ...
    if shop.updated:
        $ shopgirl("有商队来了，我们进了点新货！进来看看！")
        $ shop.updated = False

    else:
        $ shopgirl("嗨,帅哥!来看看我的商品…… *眨眼*{w=1.0}{nw}")
