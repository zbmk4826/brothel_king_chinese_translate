...
#    stop music fadeout 2.0
    ...
    if result > 1 and dice(6) >= 5:
        ...
        if game.chapter >= 3 and dice(6) >= 5:
            $ MC.items.append(search_items("西梅利亚文物")[0])
            play sound s_vibro
            "You have received a rare {b}Cimerian artefact{/b}."

        else:
            $ MC.items.append(search_items("西梅利亚废料")[0])
            play sound s_vibro
...
#    stop music fadeout 2.0
    ...
    # Pick challenge
    $ tt = show_tt("top_right")
    $ chal = renpy.call_screen("challenge_menu", challenges=[("修复货车", "force", selected_district.rank + 3), ("带来一些客户", "rally", selected_district.rank+3)], cancel=("拒绝帮助", False))
    hide screen tool
